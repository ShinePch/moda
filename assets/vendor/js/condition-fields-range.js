...
const CONDITION_FIELDS_RANGE = {
  ...
  range_index_change: {
    ...
    buildLabel: function (v) {
      return v.mode === '범위'
        ...
        : v.index_type + ' 등락률 ' + v.value1 + '% ' + (v.direction || '이상');
    }
  },
  range_search_time: {
    label: '검색시간',
    ...
    buildLabel: function (v) {
      return (
        ...
        v.hour +
        ':' +
        String(v.min_s).padStart(2, '0') +
        ' ~ ' +
        v.hour +
        ':' +
        String(v.min_e).padStart(2, '0') +
        ':' +
  ...
  range_capital: {
    label: '자본금',
    fields: [
      { id: '_capital_basis', type: 'static_label', text: '최근결산' },
      {
        id: 'mode',
        type: 'mode_radio_pair',
        unit: '억원',
    ...
    buildLabel: function (v) {
      return v.mode === '범위'
        ...
        : '자본금 ' + Number(v.value1).toLocaleString() + '억원 ' + (v.direction || '이상');
    }
  },
  range_shares: {
    label: '상장주식수',
  ...
  range_mktcap: {
    label: '시가총액',
    fields: [
      { id: '_mktcap_basis', type: 'static_label', text: '현재가기준' },
      {
        id: 'mode',
        type: 'mode_radio_pair',
        unit: '십억원',
    ...
    buildLabel: function (v) {
      return v.mode === '범위'
        ...
        : '시가총액 ' + Number(v.value1).toLocaleString() + '십억원 ' + (v.direction || '이상');
    }
  },
  range_parvalue: {
    label: '액면가',
  ...
  range_listdate: {
    ...
    buildLabel: function (v) {
      return '상장일 ' + Number(v.days).toLocaleString() + '일(영업일) 이내';
    }
  },
  range_margin: {
    label: '증거금률',
